...
function certificatePostScriptum(recipientInfo) {
    let str = "<p>avendo registrato " + recipientInfo.numOreReg + " ore di presenza, ";
 ...
 */
async function main() {
    const recipientsList = (await readJsonFile('destinatari.json')).filter(p => p.numOreReg >= HOURS_TO_BE_CERTIFICATED);
