...
async function sendPersonalizedEmails(transporter, recipients, htmlTemplate, subject) {
    ...
    for (const recipientInfo of recipients) {
        ...
        const cf = getRandomInt(10000, 50000);
        const cv = getRandomInt(10, 150);
        const p = getRandomInt(51, 100);
        const R = getRandomInt(1000, 4000);

        const personalizedHtml = mustache.render(htmlTemplate, { lettera, nome, cf, cv, p, R });
